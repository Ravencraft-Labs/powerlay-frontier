"""types target — emit data/raw/types.json shape via real CCP typesLoader."""
...
class TypesTarget:
    ...
    def extract(self, client: ClientRoot, run_dir: Path) -> ExtractResult:
        ...
        for type_id in raw:
            ...
            entry["typeID"] = int(type_id)
            name_id = entry.get("typeNameID")
            entry["typeName_en-us"] = localizer.get(name_id if isinstance(name_id, int) else None)
            output[str(int(type_id))] = entry
