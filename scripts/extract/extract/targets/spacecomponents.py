"""spacecomponents target — emit raw record set verbatim.

Strip-data.js filters the entries that have `assemblyConstruction` at consume
time; we emit everything via `record_to_dict` to keep that downstream
flexibility.
"""
...
class SpaceComponentsTarget:
    ...
    def __init__(self, *, loader_factory: Optional[Callable[[ClientRoot], Any]] = None):
    ...
    def extract(self, client: ClientRoot, run_dir: Path) -> ExtractResult:
        ...
        for type_id in raw:
            rec = raw[type_id]
            output[str(int(type_id))] = record_to_dict(rec)
